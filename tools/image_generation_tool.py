...
IMAGE_GENERATE_SCHEMA = {
    ...
    "description": (
        ...
        "for text-to-image. Each successful call is billed by the image "
        "provider — do not regenerate for grayscale, phone preview, crop, "
        "resize, watermark, or near-identical variants; do those locally "
        "(Pillow/terminal) and overwrite one stable FINAL artifact instead. "
        "Prefer `image_url` edit over a fresh text-to-image when refining an "
        "existing image. The underlying backend (FAL, OpenAI, xAI, etc.) "
