...
VERXIO_CREDENTIAL_GUIDANCE = (
    ...
    "(Image Generation and Video Generation use DashScope / Qwen Cloud when "
    "DASHSCOPE_API_KEY is present — not FAL unless they chose FAL). They can "
    "also save keys under Settings → Tools & Keys → Tools (use Add custom key "
    "for vars not listed). Provider/LLM keys live under Settings → Providers → "
    "API keys. After saving, credentials reload automatically; they should "
