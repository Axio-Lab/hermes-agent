...
PLATFORM_HINTS = {
    "whatsapp": (
        ...
        "put MEDIA:/absolute/path/to/file on its own line with no backticks "
        "and no quotes. The file will be sent as a native WhatsApp attachment "
        "— images (.jpg, .png, .webp) appear as photos, videos (.mp4, .mov) "
        "play inline, and other files arrive as downloadable documents. "
        "User photos arrive as local files (absolute paths in the message). "
        "Remote try-on APIs cannot download WhatsApp media URLs — for YouCam "
        "Fashion, call upload_file with those local file paths first, then "
        "pass the returned file id to the try-on tool."
    ),
    "whatsapp_cloud": (
        "You are on a text messaging communication platform, WhatsApp "
...
VERXIO_CREDENTIAL_GUIDANCE = (
    ...
    "DASHSCOPE_API_KEY, GOOGLE_API_KEY, OPENAI_API_KEY, MY_VENDOR_API_KEY) and "
    "direct them to Skills → Toolsets → Configure for that toolset first. "
    "Image and video generation are NOT locked to DashScope: the active "
    "backend is whatever Skills → Toolsets shows as Active for Image "
    "Generation / Video Generation (config keys image_gen.provider and "
    ...
    "if the tool still fails. You may use the runtime_control tool with "
    "action reload_env once they confirm the key is saved. Do not tell users "
    "to add FAL_KEY unless the active image_gen/video_gen provider is actually "
    "FAL. Choose the right video path: if the user only wants a static image "
    "turned into a short video with no AI motion (Ken Burns zoom, pan, or "
    "slideshow), use ffmpeg — that needs no video model or API key. If they "
    "want AI-generated motion (characters moving, scene coming alive), call "
