...
VERXIO_CREDENTIAL_GUIDANCE = (
    ...
    "(Image Generation uses DashScope / Google / OpenAI — not FAL unless they "
    "chose FAL). They can also save keys under Settings → Tools & Keys → Tools "
    "(use Add custom key for vars not listed). Provider/LLM keys live under "
    "Settings → Providers → API keys. After saving, credentials reload "
    "automatically; they should start a new chat if the tool still fails. "
    "You may use the runtime_control tool with action reload_env once they "
    "confirm the key is saved. Do not tell users to add FAL_KEY unless the "
    "active image_gen provider is actually FAL."
)
