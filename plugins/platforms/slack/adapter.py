...
class SlackAdapter(BasePlatformAdapter):
    ...
    async def connect(self) -> bool:
        ...
        try:
            ...
            # Verxio-branded manifests register /verxio instead of /hermes.
            if "verxio" not in _slash_names:
                _slash_names.append("verxio")
            if _slash_names:
    ...
    async def create_handoff_thread(
        ...
        try:
            ...
            if client is None:
                return None
            seed_text = (
                f":thread: Verxio handoff — *{(name or 'session').strip()[:80]}*"
            )
    ...
    async def _handle_slash_command(self, command: dict) -> None:
        ...
        if slash_name in {"hermes", "verxio", ""}:
            # Legacy /hermes (or Verxio-branded /verxio) <subcommand> [args]
            # routing + free-form questions.
    ...
    def _write_slack_manifest_and_instruct() -> None:
        ...
        try:
            ...
            manifest = _build_full_manifest(
                bot_name="Verxio",
                bot_description="Your Verxio agent on Slack",
            )
            ...
            print_info(
                "   Re-run the Slack manifest generator anytime to refresh after "
                "Verxio adds new commands."
            )
        except Exception as e:
