...
def _cron_dir() -> Path:
    """Cron directory for the *current* Hermes home.

    Multiplexed Verxio workers serve many tenants from one process with a
    context-local home override (``hermes_constants.set_hermes_home_override``);
    in that case the tenant's ``{home}/cron`` must win over the module-level
    ``CRON_DIR`` that was resolved at import time. Without an override the
    module globals are returned unchanged so the dashboard's per-profile swap
    (``hermes_cli.web_server._call_cron_for_profile``) keeps working.
    """
    from hermes_constants import get_hermes_home_override

    override = get_hermes_home_override()
    if override:
        return Path(override) / "cron"
    return CRON_DIR


def _jobs_file() -> Path:
    override_dir = _cron_dir()
    if override_dir is CRON_DIR:
        return JOBS_FILE
    return override_dir / "jobs.json"
