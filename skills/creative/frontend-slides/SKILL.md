...
description: "Create stunning HTML presentations from scratch or PowerPoint — zero-dependency, visual style discovery."
platforms: [linux, macos, windows]
metadata:
  hermes:
    tags: [presentations, slides, html, design, ppt]
    category: creative
    related_skills: [frontend-design, powerpoint]
---
...
Load when the user wants to build a web presentation, pitch deck, talk slides, convert PPT/PPTX to HTML, or create beautiful zero-dependency slide decks. Trigger on: make slides, HTML presentation, pitch deck, convert PowerPoint, presentation design, slide deck, or `/frontend-slides`.
...
1. **Extract content** — Run `uv run python3 SKILL_DIR/scripts/extract-pptx.py <input.pptx> <output_dir>` (install python-pptx if needed: `uv pip install python-pptx`)
